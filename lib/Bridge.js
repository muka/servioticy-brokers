...
Bridge.prototype.restart = function(then) {
    ...
    if(this.restartCounter > 5) {
        this.logger.error("Restarting to fast, stopping process");
        process.exit(1);
    }
...
Bridge.prototype.handleResponse = function(attr) {
    ...
    this.publish('/topic/' + attr.authorization + ".to", attr.message, attr.headers);
};
...
//        me.logger.debug("Going for a " + request.meta.method);
//        me.logger.debug("Posted data " + JSON.stringify(request.body));
        ...
        me.request(req, function(err, data, response) {
            ...
            if(err) {
                me.logger.debug("Aborted subscription publish");
                return;
            }

            // send back the identifier if any, allowing the requesting client to match the response
            me.logger.debug("input message id: " + request.meta.messageId);
